...
class Point():
    ...
    def get_relative_angle(self, p):
        ...
        relative_r = np.sqrt(relative_x**2 + relative_y**2)        
...
class Polygon(Figure):
    ...
    def __init__(self, vertices):
        if len(vertices) < 3:
            return None
        self.number_of_verticles = len(vertices)
    ...
    def is_convex(self):
        for i in range(self.number_of_verticles):
            relative_angles = np.array([self.vertices[i].get_relative_angle(self.vertices[j]) 
                                        for j in range(i+1, self.number_of_verticles)])
    ...
    def is_point_inside(self, p): 
        ...
        if self.left_border < p.x < self.right_border and self.bottom_border < p.y < self.top_border:              
            for i, edge in enumerate(self.edges):
                if edge.is_point_in_line(p):
                    return False

                line = edge.get_line_to_point(p)

                if edge.is_point_in_line(line.ending_point):
                    counter +=1
                    for interior_edge in self.edges:
                        CP = line.get_crossing_point(interior_edge)
                        if interior_edge.is_point_in_line(CP) and line.is_point_in_line(CP) and \
                                line.starting_point != CP and line.ending_point!= CP:
                            return False
    ...
    def is_point_inside_method_1(self, p): 
    ...
    def get_perimeter(self):        
        return round(sum(edge.get_lenght() for edge in self.edges), 2)
